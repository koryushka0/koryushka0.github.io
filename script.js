    ...
    if (document.body.id === 'contact-page') {
        ...
        if (contactForm) {
            contactForm.addEventListener('submit', async (e) => {
                ...
                const tgInput = document.getElementById('contact-tg-username');
                const questionInput = document.getElementById('contact-question');
                
                // Валидация
                let isValid = true;
                if (tgInput.value.trim() === '') {
                    tgInput.parentElement.querySelector('.error-message').textContent = 'Укажите ваш юзернейм в Telegram';
                    isValid = false;
                } else {
                    tgInput.parentElement.querySelector('.error-message').textContent = '';
                }
                if (questionInput.value.trim() === '') {
                    questionInput.parentElement.querySelector('.error-message').textContent = 'Пожалуйста, напишите ваш вопрос';
                    isValid = false;
                } else {
                    questionInput.parentElement.querySelector('.error-message').textContent = '';
                }
                if (!isValid) return;
                ...
                try {
                    const response = await fetch('https://klas0.pythonanywhere.com/submit-question', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                            name: nameInput.value,
                            username: tgInput.value,
                    ...
                    contactForm.innerHTML = '<h4 style="text-align:center; color: var(--primary);">Спасибо! Ваш вопрос отправлен. (. ❛ ᴗ ❛.)<br>Мы ответим вам прямо в Telegram.</h4>';
                    ...
                    showNotification(`Ошибка: ${error.message}`);
                    submitButton.disabled = false;
                    submitButton.textContent = 'Отправить вопрос';
                }
            });
        }
    }
